...
public:
	...
	void set(std::string path, sf::Vector2f& offset, std::shared_ptr<ResourceManager<sf::Texture>>, unsigned index);
	...
	void updatePos(std::pair<unsigned, unsigned> newpos, BoardState& state);
	friend bool operator<(const DrawComponent& l, const DrawComponent& r) { return l.zval < r.zval; }
};